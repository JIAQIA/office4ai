...
uv run python tests/integration_tests/test_word_e2e.py --mode health

# 方式 2: 使用交互式启动脚本
uv run python tests/integration_tests/test_workspace_startup.py
```
...
1. **启动端到端测试**
   ```bash
   uv run python tests/integration_tests/test_word_e2e.py --mode e2e
   ```
