"""
Basic Get Document Structure Test
...
async def get_document_structure(
    ...
    Returns:
        Optional[dict]: 文档结构数据，如果失败则返回 None
    """
    if test_name:
        print(f"\n📝 {test_name}")
    ...
    if result.success:
        ...
        if result.data:
            ...
            print(f"   表格数: {result.data.get('tableCount', 'N/A')}")
            print(f"   图片数: {result.data.get('imageCount', 'N/A')}")
            print(f"   节数: {result.data.get('sectionCount', 'N/A')}")
        return result.data
    else:
...
async def test_empty_document_structure():
    """测试 1: 空白文档结构"""
    return await run_test_template(
        test_name="空白文档结构",
        test_number=1,
        expected_description="空白文档应该有 0 个段落（或 1 个空段落），0 个表格，0 个图片，1 个节",
    )


async def test_simple_document_structure():
    """测试 2: 简单文档结构"""
    return await run_test_template(
        test_name="简单文档结构",
        test_number=2,
        expected_description="简单文本文档应该有多个段落，0 个表格，0 个图片",
    )
...
async def test_complex_document_structure():
    ...
    return await run_test_template(
        test_name="复杂文档结构",
        test_number=3,
        expected_description="包含表格和图片的文档应该正确统计各元素数量",
    )


async def test_multi_paragraph_document_structure():
    """测试 4: 多段落文档结构"""
    return await run_test_template(
        test_name="多段落文档结构",
        test_number=4,
        expected_description="长文档应该正确统计大量段落数量（性能测试）",
    )
...
async def run_all_tests():
    ...
    print("\n⚠️  请准备: 空白 Word 文档")
    input("按回车继续...")
    results.append(await test_empty_document_structure())
    await asyncio.sleep(2)

    # 测试 2: 需要准备简单文档
    print("\n⚠️  请准备: 包含几段简单文本的 Word 文档")
    input("按回车继续...")
    results.append(await test_simple_document_structure())
    await asyncio.sleep(2)

    # 测试 3: 需要准备复杂文档
    print("\n⚠️  请准备: 包含表格和图片的 Word 文档")
    input("按回车继续...")
    results.append(await test_complex_document_structure())
    await asyncio.sleep(2)

    # 测试 4: 需要准备长文档
    print("\n⚠️  请准备: 包含大量段落的 Word 文档（50+ 段落）")
    input("按回车继续...")
    results.append(await test_multi_paragraph_document_structure())
...
# 测试映射表：用于命令行参数路由
TEST_MAPPING = {
    "1": test_empty_document_structure,
    "2": test_simple_document_structure,
    "3": test_complex_document_structure,
    "4": test_multi_paragraph_document_structure,
    "all": run_all_tests,
}
...
if __name__ == "__main__":
    ...
    parser = argparse.ArgumentParser(description="Basic Get Document Structure E2E Tests")
    parser.add_argument(
        "--test",
        choices=["1", "2", "3", "4", "all"],
        default="1",
        help="Test to run: 1=empty, 2=simple, 3=complex, 4=multiparagraph, all=all tests",
    )
